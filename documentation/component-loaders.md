 ...
 * `ko.components.register(name, configuration)`
   * Registra um componente. Veja: [documentação completa](component-registration.html).

 * `ko.components.isRegistered(name)`
   * Retorna `true` se um componente com o nome especificado já foi registrado; senão retorna `false`.
...
# Implementando um componente loader customizado {#custom-component-loader}
...
## Funções que você pode implementar

Um component loader customizado é simplemente um objeto cujas propriedades são **qualquer combinação** das seguintes funções:
...
## Registrando componentes loaders customizados
...
# Exemplo 1: Um componente loader que configura convenções de nomenclatura

Para implementar uma converção de nomenclatura, seu componente loader customizado só precisa implementar o `getConfig`. Por exemplo:
...
# Exemplo 2: Um componente loader que carrega arquivos externos usando código customizado
...
# Nota: Componentes loader customizados e elementos customizados
...
Veja: [Como habilitar elementos personalizados com nomes que não correspondem aos componentes explicitamente registrado](component-custom-elements.html#registering-custom-elements)

# Nota: Integrando com browserify
